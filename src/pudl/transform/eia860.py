...
class Generators(Tf):
    ...
    def clean(gp_df, ge_df, gr_df):
        ...
        return dfs.dropna(subset=['generator_id', 'plant_id_eia']).pipe(pudl.helpers.fix_eia_na)

    def transformed(gens_df):
...
class Plant(Tf):
    def transformed(p_df):
        ...
        p_df.liquefied_natural_gas_storage = \
            p_df.liquefied_natural_gas_storage.replace(
                to_replace='X', value='N')
